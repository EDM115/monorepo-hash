...
`src/go/monorepo-hash.go` is the Go implementation of the CLI. It should be as close as possible to the original one, but some differences may be acceptable if they are justified by the language differences. Behavior must be identical, but not all functions should behave the same. The goal is to have something at least a little bit idiomatic to Go, while still being as close as possible to the original. It replaced the Bun binary starting v3.0.0.  
`src/rust/monorepo-hash.rs` is the Rust implementation of the CLI. It should follow the exact same user-facing behavior and shared test expectations as the Go binary, while remaining reasonably idiomatic to Rust where that does not affect compatibility.  
...
`tests/go` and `tests/rust` only run the shared tests from `harnesses`.
...
Dependencies in the `package.json` are shared between the Node implementation. For Go, see `src/go/go.mod`. For Rust, see `src/rust/Cargo.toml`.  
For building the binaries, we have a custom script (`build.script.ts`) that calls `go` or `cargo` directly and crafts the right arguments to pass.  
