...
describe("hash generation", () => {
  ...
  beforeAll(async () => {
    ...
    cli = join(cwd, "go", "monorepo-hash.exe")
...
`
    ...
    await mkdirp(join(demoDir, "go"))

    await copyFile(join(cwd, "go", "monorepo-hash-linux-x64"), join(demoDir, "go", "monorepo-hash-linux-x64"))
    await copyFile(join(cwd, "go", "monorepo-hash.exe"), join(demoDir, "go", "monorepo-hash.exe"))
  })
