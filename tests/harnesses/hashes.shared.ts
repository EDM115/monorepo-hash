...
export function defineHashesSuite(runCli: RunCli): void {
  let cwd: string
  let demoDir: string
...
`
  ...
  describe("unified", () => {
    ...
    it("generates hash for a single workspace", async () => {
      ...
      await runCli(demoDir, [ "--generate", "--target=packages/cli-tools" ])
    ...
    it("produces the same hash for a workspace with transitive deps as in full generate", async () => {
      ...
      // partial generate
      await runCli(demoDir, [ "--generate", "--target=services/backend" ])
      // oxlint-disable-next-line no-unsafe-type-assertion
  ...
  describe("workspaces", () => {
    it("generates all hashes and matches snapshot", async () => {
      await runCli(demoDir, [ "--generate", "--workspaces" ])
    ...
    it("generates hash for a single workspace", async () => {
      ...
      await runCli(demoDir, [ "--generate", "--target=packages/cli-tools", "--workspaces" ])
    ...
    it("produces the same hash for a workspace with transitive deps as in full generate", async () => {
      ...
      // partial generate
      await runCli(demoDir, [ "--generate", "--target=services/backend", "--workspaces" ])
